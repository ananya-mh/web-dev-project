...
//                         {user.role} </div>)}
//                 {user && editing && (
//                     <input className="form-control w-50 wd-edit-roles"
...
export default function PeopleDetails() {
    ...
    const roles = ["Admin", "Faculty", "Student", "Assistants"];
    ...
    const saveUser = async () => {
        const [firstName, lastName] = name.includes(" ") ? name.split(" ") : [name, ""];
    ...
    return (
        <div className="wd-people-details position-fixed top-0 end-0 bottom-0 bg-white p-4 shadow w-25">
            ...
            <div className="wd-email">
                ...
                {!isEditingEmail ? (
                    <div onClick={() => setIsEditingEmail(true)}>{user.email}</div>
                ) : (
                    <input
                        type="email"
            ...
            <div className="wd-roles">
                ...
                {!isEditingRole ? (
                    <div onClick={() => setIsEditingRole(true)}>{user.role}</div>
                ) : (
                    <select
